  ...
  const Odometry & current_kinematics)
{
  ...
  if (m_use_temporal_trajectory) {
    ...
    if (!success_resample) {
      outputMessage(MessageId::spline_resample_failed);
      return;
    }
  ...
  if (
    ...
    if (
      ...
      outputMessage(MessageId::path_filter_failed);
  ...
  if (mpc_traj_smoothed.yaw.size() != mpc_traj_smoothed.vx.size()) {
    outputMessage(MessageId::trajectory_size_inconsistent);
  }
  ...
  if (!mpc_traj_smoothed.size()) {
    outputMessage(MessageId::resampled_trajectory_empty);
    return;
  }
...
VectorXd MPC::getInitialState(const MPCData & data)
{
  ...
  if (vehicle_model == "kinematics") {
    ...
    outputMessage(MessageId::state_vehicle_model_undefined);
  }
  return x0;
}
  ...
  const MPCTrajectory & traj, const double & start_time, const VectorXd & x0_orig)
{
  ...
  for (size_t i = 0; i < m_input_buffer.size(); ++i) {
    ...
    try {
      ...
      outputMessage(MessageId::delay_compensation_resample_failed, e.what());
  ...
  const MPCTrajectory & reference_trajectory, const double dt, const std::string & coordinate) const
{
  ...
  if (coordinate == "world") {
    if (m_vehicle_model_ptr->modelName() == "dynamics") {
      outputMessage(MessageId::world_coordinate_prediction_unsupported);
    }
