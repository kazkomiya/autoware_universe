  ...
  for (const auto & message : messages) {
    switch (message.id) {
      ...
      case MessageId::mpc_failed:
        RCLCPP_ERROR(logger_, "MPC failed due to %s", message.detail.c_str());
        break;
      case MessageId::stopped_state_detected:
        RCLCPP_DEBUG_THROTTLE(
          logger_, *clock_, 5000, "Stopped state detected, use previous control command");
        break;
      case MessageId::mpc_not_solved:
        RCLCPP_DEBUG_THROTTLE(
          logger_, *clock_, 5000, "MPC is not solved, use stop control command");
        break;
      case MessageId::trajectory_shape_changed:
        RCLCPP_DEBUG(logger_, "trajectory shaped is changed");
        break;
      case MessageId::no_vehicle_model:
        RCLCPP_INFO_THROTTLE(logger_, *clock_, 5000, "MPC does not have a vehicle model");
        break;
      case MessageId::no_qp_solver:
        RCLCPP_INFO_THROTTLE(logger_, *clock_, 5000, "MPC does not have a QP solver");
        break;
      case MessageId::reference_trajectory_empty:
        RCLCPP_INFO_THROTTLE(logger_, *clock_, 5000, "trajectory size is zero.");
        break;
      ...
      case MessageId::steering_not_converged:
        RCLCPP_DEBUG_THROTTLE(logger_, *clock_, 5000, "steering is not converged.");
        break;
    }
  }
}
