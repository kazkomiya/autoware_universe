...
private:
  ...
  inline void outputMessage(const MessageId id) const { m_messages.push_back({id, {}}); }

  //!< @brief keep a message whose text the control received from elsewhere
  inline void outputMessage(const MessageId id, std::string detail) const
  {
